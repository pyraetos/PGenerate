	...
	public void layer(int layer){
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				...
				case(1): if(pg.getValue(i, j) > .75) pixels[i * height + j] = OCP_LIGHT_GREEN.getRGB(); break;
				case(2): if(pg.getValue(i, j) > 1) pixels[i * height + j] = OCP_GREEN.getRGB(); break;			
				case(3): if(pg.getValue(i, j) > 1.35) pixels[i * height + j] = OCP_DARK_BROWN.getRGB(); break;			
